                ...
                success: function (data) {
                    if (selectBy == 'state') {
                        ...
                        for (var i = 0; i < data.tts.length; i++) {
                            ...
                            newjson[i].working = data.tts[i].working;
                            newjson[i].retired = data.tts[i].retired;
                        }
                        var option = {
                            ...
                            series: [
                                {
                                    ...
                                    data: [newjson[0].working, newjson[1].working, newjson[2].working, newjson[3].working, newjson[4].working, newjson[5].working,
                                        newjson[6].working, newjson[7].working, newjson[8].working, newjson[9].working],
                                    //系列中的数据标注内容
                                    markPoint: {
                                        data: [
                                            {type: 'max', name: '最大值'},
                                ...
                                },
                                {
                                    ...
                                    type: 'bar',
                                    //系列中的数据内容数组，折线图以及柱状图时数组长度等于所使用类目轴文本标签数组axis.data的长度，并且他们间是一一对应的。数组项通常为数值
                                    data: [newjson[0].retired, newjson[1].retired, newjson[2].retired, newjson[3].retired, newjson[4].retired, newjson[5].retired,
                                        newjson[6].retired, newjson[7].retired, newjson[8].retired, newjson[9].retired],
                                    //系列中的数据标注内容
                                    markPoint: {
                                        data: [
                                            {type: 'max', name: '最大值'},
                        ...
                        for (var i = 0; i < data.tts.length; i++) {
                            newjson[i] = new Object();
                            newjson[i][selectBy] = data.tts[i][selectBy];
                        }
                        option = {
                            title: {
                                text: '泸州市医保基金预测'
                        ...
                        option.series.push({
                            ...
                            type: 'line',
                            stack: '总量',
                            data: [newjson[0][selectBy], newjson[1][selectBy], newjson[2][selectBy], newjson[3][selectBy], newjson[4][selectBy], newjson[5][selectBy],
                                newjson[6][selectBy], newjson[7][selectBy], newjson[8][selectBy], newjson[9][selectBy]]
                        });
                        line.setOption(option);
                    }
                }
        ...
        $.ajax({
                ...
                success: function (data) {
                    ...
                    function dataFormatter(obj) {
                        var pList = ['一等级','天津','河北','山西','内蒙古','辽宁','吉林','黑龙江','上海','江苏','浙江','安徽','福建','江西','山东','河南','湖北','湖南','广东','广西','海南','重庆','四川','贵州','云南','西藏','陕西','甘肃','青海','宁夏','新疆'];
                        var temp;
